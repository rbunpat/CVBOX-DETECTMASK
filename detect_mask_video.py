...
#   token = 'YOURTOKENHERE' # Line Notify Token
#   img = {'imageFile': open('no_mask.png','rb')} #Local picture File
...
while True:
	...
	for (box, pred) in zip(locs, preds):
		...
		if (mask > withoutMask):
			label = "Mask"
			color = (0, 255, 0)
			# #capture the image and store it in a png file
			# cv2.imwrite("mask.png", frame)
		else:
			label = "No Mask"
			color = (0, 0, 255)
			# capture the image and store it in a png file
			# cv2.imwrite("no_mask.png", frame)
			# linenotify(
			fileindir = len([name for name in os.listdir(DIR) if os.path.isfile(os.path.join(DIR, name))])
